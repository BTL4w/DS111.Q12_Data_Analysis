"""
Update database from ALL JSON files in data/raw
Add new data or update existing data from crawl results
...
def load_existing_data(db: DatabaseV2) -> tuple:
    """Load existing product and seller IDs from database"""
    ...
    return existing_products, existing_sellers


def process_and_store_products_batch(results: dict, db: DatabaseV2, logger, 
                                     existing_products: set, existing_sellers: set):
    """Process crawl results and store/update in database with batch operations"""
    ...
    for product in all_products:
        ...
        try:
            ...
            is_new_product = product_id not in existing_products
            ...
            # Collect history data (always add new records)
    ...
    try:
        ...
        logger.info(f"  Batch inserted: {successful} products ({new_products} new, {updated_products} updated), {len(new_sellers)} sellers")
        
    except Exception as e:
        logger.error(f"Batch insert failed: {e}")
        raise
    
    return successful, failed, new_products, updated_products


def update_database_from_all_json(json_files: list, db_path: str, logger):
    """Update database from all JSON files"""
    logger.info("=" * 70)
    logger.info("UPDATING DATABASE FROM ALL JSON FILES")
    logger.info("=" * 70)
    ...
    existing_products, existing_sellers = load_existing_data(db)
    logger.info(f"  Existing products: {len(existing_products)}")
    logger.info(f"  Existing sellers: {len(existing_sellers)}")
    logger.info("")
    ...
    try:
        for idx, json_file in enumerate(json_files, 1):
            ...
            try:
                ...
                # Process and store/update products
                successful, failed, new, updated = process_and_store_products_batch(
                ...
                logger.info(f"  ✓ Completed: {successful} products ({new} new, {updated} updated), {failed} failed")
        ...
        logger.info(f"Total products processed: {total_successful}")
        logger.info(f"  - New products: {total_new}")
        logger.info(f"  - Updated products: {total_updated}")
        logger.info(f"Total products failed: {total_failed}")
        logger.info(f"Unique products in database: {len(existing_products)}")
        logger.info(f"Unique sellers in database: {len(existing_sellers)}")
        logger.info(f"Database: {db_path}")
        logger.info("=" * 70)
