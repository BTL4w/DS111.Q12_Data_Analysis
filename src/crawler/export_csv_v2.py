        ...
        try:
            ...
            if not df.empty:
                ...
                df['brand_name'] = df['brand'].apply(
                    lambda x: json.loads(x).get('name', '') if x else ''
                )
                ...
                df['badge_names'] = df['badges'].apply(
                    lambda x: ', '.join([b.get('name', '') for b in json.loads(x)]) if x else ''
                )
    ...
    def export_latest_snapshot(self, filename: Optional[str] = None) -> str:
        ...
        try:
            ...
            if not df.empty:
                ...
                df = df.drop(columns=['brand', 'badges'])
